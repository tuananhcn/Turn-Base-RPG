...
@export var debuff_defense_percentage: = 0.2  # Reduces defense by 20%
...
func execute(source: Battler, targets: Array[Battler] = []) -> void:
	...
	var damage = calculate_damage(attack_power, base_damage, defense_power)
	...
	print(target.stats.defense)
	# Optionally, apply an attack buff to the source
